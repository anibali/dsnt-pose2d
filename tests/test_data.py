...
class TestMPIIDataset(TestCase):
    def test_len(self):
        test_set = MPIIDataset('/datasets/mpii', 'test')
        test_set_len = 11731
        self.assertEqual(test_set_len, len(test_set))

        train_set = MPIIDataset('/datasets/mpii', 'train')
        train_set_len = 25925
        self.assertEqual(train_set_len, len(train_set))

    def test_getitem(self):
        dataset = MPIIDataset('/datasets/mpii', 'train', use_aug=False)
        sample = dataset[543]
    ...
    def test_image_specs(self):
        ...
        dataset = MPIIDataset(
            '/datasets/mpii', 'train', use_aug=False, image_specs=image_specs)
        sample = dataset[42]
